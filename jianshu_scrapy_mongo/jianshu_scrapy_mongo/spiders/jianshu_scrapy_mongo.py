...
class jianshuSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        """
        info_item=JianshuScrapyMongoItem()
        ...
        gender='to-do'
        following_num=response.xpath('/html/body/div[1]/div/div[1]/div[1]/div[3]/ul/li[1]/div/a/p/text()').extract_first()
        follower_num=response.xpath('/html/body/div[1]/div/div[1]/div[1]/div[3]/ul/li[2]/div/a/p/text()').extract_first()
        articles_num=response.xpath('/html/body/div[1]/div/div[1]/div[1]/div[3]/ul/li[3]/div/a/p/text()').extract_first()
    ...
    def parser_followering(self,response):
        ...
        followering_num=int(response.xpath('/html/body/div[1]/div/div[1]/ul/li[1]/a/text()').extract_first().split(' ')[-1])
        range_end=10	
        if followering_num < followers_per_page:
            range_end=followering_num
        for i in range(1,range_end):
            followering_uid=response.xpath('/html/body/div[1]/div/div[1]/div[2]/ul/li[{id}]/div[1]/a/@href'.format(id=i)).extract_first().split('/')[-1]	
            #print ( "http://www.jianshu.com/u/{uid}".format(uid=followering_uid) )
            yield Request( "http://www.jianshu.com/u/{uid}".format(uid=followering_uid),headers=self.base_headers,callback=self.parse)
    ...
    def parser_followers(self, response):
        ...
        author_uid=response.url.split('/')[-2]	
        followers_per_page=9
        followers_num=response.xpath('/html/body/div/div[1]/div[1]/div[1]/div[3]/ul/li[2]/div/a/p/text()').extract_first()
        ...
        for i in range(1,range_end):
            ...
            yield Request( "http://www.jianshu.com/u/{uid}".format(uid=follower_uid),headers=self.base_headers,callback=self.parse)
